...
async function loadPluginFactory(name: string, repositoryRoot: string): Promise<PluginFactory> {
	...
	try {
		...
		if (process.env.JEST_WORKER_ID !== undefined) {
			...
			return (require(importPath) as unknown as {
				default: PluginFactory,
			}).default;
		} else {
			return ((await import(importPath)) as unknown as {
				default: PluginFactory,
			}).default;
		}
	} catch (err) {
